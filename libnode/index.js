 ...
 */
async function* mixLoader(iterators, sortFn, pickNumber) {
  // 结果集, 用于保存要返回的数据
  const dataSet = []; // 用于保存待消费数据的优先队列

  const dataQueue = new _PriorityQueue.default((a, b) => sortFn(a.value, b.value)); // 数据源列表
  ...
  while (dataSources.length > 0 || dataQueue.length) {
    ...
    const {
      drySrc = [],
      dryingSrc = []
    } = (0, _lodash.default)(dataSources, sourceSeparator); // 对于余量为 0 的数据源
    // 策略为获取一次数据, 并需要等待返回

    const reqs = drySrc.map(src => src.req || src.iterator.next()); // 对于余量大于 0 小于 pickNumber 的数据源
    // 策略为预先获取一次数据, 但不需要等待返回

    dryingSrc.forEach(src => src.req = src.iterator.next()); // 等待所有余量为 0 的数据源返回，保证每个数据源都有数据在优先队列中
    ...
    while (dataSet.length < pickNumber && dataQueue.length) {
      ...
      dataSet.push(item.value); // 对应数据源未消费数据减少 1

      --item.src.poolSize; // 如果此时数据源待消费数据量为 0 , 且结果集数据不足, 需要等待此数据源迭代器返回新数据
  ...
  function updateData(source, result) {
    ...
    source.req = null; // 如果该迭代器未迭代完, 将数据放入优先队列中
