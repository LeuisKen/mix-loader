...
exports.default = (() => {
    var _ref = _asyncGenerator3.default.wrap(function* (iterators, sortFn, pickNumber) {
        ...
        while (dataSources.length > 0 || dataQueue.length) {
            ...
            const dryingSrc = dataSources.filter(function (src) {
                return src.poolSize > 0 && src.poolSize < pickNumber && !src.req;
            });
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                if (!done) {
                    dataQueue.push(...value.map(function (value) {
                        return { value, src: drySrc[i] };
                    }));
            ...
            while (dataSet.length < pickNumber && dataQueue.length) {
                ...
                if (!item.src.poolSize && dataSet.length < pickNumber) {
                    ...
                    const { value, done } = yield _asyncGenerator3.default.await(item.src.req);
                    item.src.req = null;
                    // 如果该迭代器未迭代完, 将数据放入优先队列中
                    if (!done) {
                        dataQueue.push(...value.map(function (value) {
